...
export default class BiPspbNavigationBarQuestionnaire extends LightningElement {
	...
	caregiverNotific = resources.CAREGIVER_NOTIFICATIONS;
	adult = false;
	...
	patientInfo() {
		...
		GET_CAREGIVER_ACCOUNT({ userId: Id, isActive: true })
			.then((patient) => {//Null data has been handled in Apex
	...
	connectedCallback() {
		...
		try {
			...
			if (this.userType !== 'undefined') {
				USER_DETAILS()
					.then((user) => {//Nullcheck has been handled in Apex
	...
	openNotificSettingUnassigned()
	{
		window.location.assign(
			this.baseUrl + this.unAssignedUrl + this.caregiverNotific
		);
	}
	...
	openCommunity() {
		...
		CHECK_COMMUNITY_USERNAME({ userId: this.userId })
			.then((result) => {//Nulldata has been handled in Apex
				if (result === true) {
					window.location.assign(
	...
	openMyFollowers() {
		...
		CHECK_COMMUNITY_USERNAME({ userId: this.userId })
			.then((result) => {//Nulldata has been handled in Apex
				if (result === true) {
					window.location.assign(
						this.baseUrl + this.unAssignedUrl + this.follower
					);
				}
				if (result === false) {
					window.location.assign(
	...
	openFollowing() {
		...
		CHECK_COMMUNITY_USERNAME({ userId: this.userId })
			.then((result) => {//Nulldata has been handled in Apex
				if (result === true) {
					window.location.assign(
						this.baseUrl + this.unAssignedUrl + this.following
					);
				}
				if (result === false) {
					window.location.assign(
