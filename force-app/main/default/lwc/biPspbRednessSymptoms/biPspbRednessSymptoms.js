...
export default class BiPspRednessSymptom extends NavigationMixin(LightningElement) {
    ...
    humanParts = []
    rednessValue = label.REDNESS_VALUE;
    ...
    recordInsertCount = 0;
    userId = label.ID;
    rednessInfo = label.REDNESS_INFO;
    ...
    initializeSessionData() {
        ...
        const myBodyParts = globalThis?.sessionStorage.getItem('redness');
        this.insertCount = globalThis?.sessionStorage.getItem('countred');
        const myBodyIntensity = globalThis?.sessionStorage.getItem('myDataintensityredness');
    ...
    processAllergyIntoleranceData(data) {
        const rednessValueNormalized = this.rednessValue.trim().toLowerCase();
        data.forEach(record => {
            ...
            if (this.carePlanTemplateName === rednessValueNormalized) {
                this.rsd = rednessValueNormalized;
    ...
    updateButtonState() {
        ...
        this.isCheckedSelectAll = this.totalElements === 30;
        this.buttonText = this.isCheckedSelectAll ? label.BODY_PARTS_REMOVE : label.BODY_PARTS_SELECT_ALL;
        this.rednessTrack = this.clickedElement === null;
    }
    ...
    handleClickForAccept() {
        ...
        if (this.humanParts.length > 0 && parseInt(this.sliderValue, 10) > 0) {
            ...
            if (this.insertCount === '1' || this.rsd === 'redness') {
            ...
            recordOperation
                .then(result => {
                    this.handleSuccess()
                    if (result) {
                        globalThis?.sessionStorage.setItem('redness', this.humanParts);
                        globalThis?.sessionStorage.setItem('myDataintensityredness', this.sliderValue);
                        ...
                        if (this.insertCount !== '1') {
                            if (typeof window !== 'undefined') {
                                const addTaskEvent = new CustomEvent('addtask', {
                                    detail: label.REDNESS_VALUE
                                });
                                this.dispatchEvent(addTaskEvent);
                            }
                            this.recordInsertCount++;
                            globalThis?.sessionStorage.setItem('countred', this.recordInsertCount.toString());
                        }
                    }
                })
                .catch(error => {
    ...
    handleSuccess() {
        ...
        const globalThis = window;
        globalThis.sessionStorage.removeItem('redness');
    }
