...
export default class BiPspbArticleSearchParent extends LightningElement {
	...
	noSearchResult = LABELS.NO_SEARCH_RESULTS;
   ...
   /**
	...
	handleSearch(event) {
		...
		let separateKeyBySpace = searchlowerval.split(' ');
		for(let i=0; i<separateKeyBySpace.length;i++){
			this.getAriclesByKeywords(separateKeyBySpace[i]);
		}
	...
	getAriclesByKeywords(searchlowerval){
		for (let i = 0; i < this.tagArticleMap.length; i++) {
			...
			if (searchlowerval.includes(tagLowerRecord) ||tagLowerRecord.includes(searchlowerval) ) {
